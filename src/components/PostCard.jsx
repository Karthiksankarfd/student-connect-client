...
import LazyLoadMedia from "./Post/LazyLoadMedia.jsx";
const PostCard = ({
  imageUrl,
  caption,
  createdAt,
  ...
  return (
    <section className="post-card shadow-white    text-black bg-white  dark:bg-black dark:text-white flex flex-col  mb-5   relative w-full max-w-full mx-auto  rounded-xl">
      {/* Header Section */}
      <div className="p-4">
        <div className="flex justify-between items-start  ">
          ...
          <div className="group relative h-fit w-fit ">
            ...
            <ul className="shadow-md shadow-slate-600 rounded-lg p-2 leading-8 absolute z-10 text-xs right-0 top-full bg-white dark text-black dark:text-gray-300 hidden group-hover:block w-[200px]">
              <li className="text-black px-2 rounded-lg">Save</li>
              <li className="text-black px-2 rounded-lg">Share</li>
              <li className="text-black px-2 rounded-lg">Block</li>
              <li className="text-black px-2 rounded-lg">View Profile</li>
            </ul>
          </div>
        </div>

        <p className="text-[12px] py-2 whitespace-pre-line ">
          ...
          {imageUrl.map((file) => (
            <SwiperSlide>
              ...
              /> : (<video  src={file}
                ...
                autoPlay
                loop
                className="h-40 w-full border border-gray-300 rounded-md object-contain"></video>)}
      ...
      <div className=" p-5">
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex space-x-4">
            <span className="flex items-center gap-x-2">
              <FaHeart
                onClick={() => like(_id, loggedInuser?._id)}
                className={`cursor-pointer ${
                  flag || likedBy.includes(loggedInuser?._id)
                    ? "text-red-500"
                    : "text-gray-300"
                }`}
              />
        ...
        <div className="">
          <div className="flex items-center">
            ...
            <p className="ml-2 text-sm text-gray-400">
              Liked by{" "}
              <span className="text-gray-600 font-bold">Rihanna Marsh</span> and{" "}
              <span className="text-white font-bold">523 others</span>
            </p>
          </div>
        </div>
        ...
        <div className="mb-2">
          <p className="text-sm">
            <span className="font-bold">Bryan Price</span> Lorem ipsum dolor sit
            amet.{" "}
