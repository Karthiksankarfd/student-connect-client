...
import FireBaseLoginBtn from "../GoogleOauth/FireBaseLoginBtn";
const StepOneEmail = () => {
  const { setSignUpUser, signUpUser, isLoading, signUpError } =
  ...
  return (
    <section className="w-full h-dvh grid place-items-center">
      <section className="signup-Step1 border-2 rounded-lg shadow-lg w-full lg:w-fit grid grid-flow-row place-content-center gap-y-5 py-5 h-full lg:h-fit bg-white">
        <h3 className="white text-start  text-[30px] text-[#0057FF]">Signup</h3>
        <div className="loader_request_status">
          {isLoading && (
            <h4 className="text-green-500 text-center">Verifying Email...</h4>
          )}
        </div>

        <div className=" w-full bg-red-200 grid  ">
          <form className=" w-full h-fit  grid place-items-start gap-y-3 p-3">
            ...
            <input
              ...
              className="text-black block w-1/2 focus:outline-none border-2 rounded-lg px-5  py-2   border-gray-300 "
            />
            <div className=" w-1/2 flex justify-start border-2 border-blue-500  rounded-lg">
              <FireBaseLoginBtn />
            </div>

            <p className="error_message text-orange-500 text-sm ">
              {signUpError}
